	...
	static {
		try {
			...
			for (Element element : elements) {
				...
				if (nodes.size() < 2) {
					throw new RuntimeException("xml转换错误,节点数据不完整");
				}
				...
				if (id < 1) {
					throw new RuntimeException("xml转换错误,id不能小于1,(" + id
							+ "小于1)");
				}
				String name = nodes.get(1).getText();
				if (!categories.add(new Category(id, name))) {
					throw new RuntimeException("xml转换错误,Id(" + id + ")重复了！");
				}
			}
		} catch (Exception e) {
